...
export default function WorkForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = isEditMode ? `${url}/api/portfolio/${work_id}` : `${url}/api/portfolio/`;
      ...
      if (response.ok) {
        ...
        setError(data.detail || `Failed to ${isEditMode ? 'update' : 'add'} work.`);
      }
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="work-form">
            ...
            <div>
              ...
              <div className="existing-thumbnails">
                {existingOtherImageUrls.map((url, index) => (
                  <img key={index} src={url} alt={`Existing other image ${index + 1}`} className="existing-thumbnail" />
                ))}
              </div>
            </div>
          )}
        ...
        <div className="form-buttons">
          ...
          <button type="submit" className="btn-primary" disabled={loading}>
            {loading ? 'Submitting...' : (isEditMode ? 'Update Work' : 'Add Work')}
          </button>
        </div>
      </form>
    </div>
